...
检查 LaTeX 公式（`$...$` 或 `$$...$$`）中是否有直接暴露的中文（未包裹在 `\text{}` 中）。
- **错误做法**：在公式中直接书写中文（如 `$v(s_{\text{急诊}})）$`，注意中文标点也容易暴露在外面；或 `$R(A, 左, \cdot)$`。
- **正确做法**：使用 `\text{}` 将中文字符包裹起来（如 `$R(A, \text{左}, \cdot)$`）。
- **注意**：KaTeX 引擎对 `\text{中文}` 的支持非常完美，**无需**将其转换成 Markdown 表格或移出公式，只需确保公式内所有中文字符均被 `\text{}` 包裹即可。
