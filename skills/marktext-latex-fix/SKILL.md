...
- **行内公式**：将中文文本移到公式外部
- **公式下标中的中文**：若公式中存在表示状态/动作等特定含义的中文下标（如 $v(s_{\text{急诊床位}})$），为避免破坏公式整体连贯性，推荐将其统一翻译为简洁的英文单词（如 $v(s_{\text{clinic}})$），并在正文中进行对照说明。
- **LaTeX 装饰符号中的中文**：对于 `\underbrace{...}_{\text{中文描述}}` 等装饰或标注块中的中文文本，在 KaTeX 中渲染极易报错，应当使用英文标注（如 `\text{immediate reward}`），再在正文中作补充说明。
