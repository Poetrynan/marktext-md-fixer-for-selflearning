...
### ⚠️ 关键防错：内容完整性比对（防信息丢失）

当使用 Python 脚本或高精度编辑工具批量替换或重构 LaTeX 公式和表格时，**极易因为过度关注语法修复（如定界符转换、竖线 `|` 转 `\mid`）而漏掉公式周围或表格其它单元格的说明性文字**。
- **强制要求**：完成修改后，必须运行 `git diff` 或将修改前后的文本进行仔细比对，确认**所有原始信息（特别是中文解释、表格附加列等）均被完好保留**，严禁在排版优化过程中发生非故意的内容丢失。
